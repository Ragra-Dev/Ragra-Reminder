...
def test_repeated_full_sync_does_not_duplicate_courses_or_tasks(conn):
    ...
    summary3 = sync_classroom(conn, client, user_id=owner_id(conn))
...
def test_deadline_change_updates_task_and_recomputes_reminders(conn):
    ...
    sync_classroom(conn, client_v1, user_id=owner_id(conn))
    ...
    summary = sync_classroom(conn, client_v2, user_id=owner_id(conn))
...
def test_non_active_or_provisioned_course_states_never_generate_reminders(conn, non_eligible_state):
    ...
    assert summary.courses_seen == 0
    assert conn.execute("SELECT COUNT(*) AS c FROM tasks").fetchone()["c"] == 0
    due = repo.due_pending_reminders(conn, now="2026-12-31T00:00:00+00:00", user_id=owner_id(conn))
    assert due == []
...
def test_previously_active_course_going_archived_updates_stored_state(conn):
    ...
    sync_classroom(conn, client_active, user_id=owner_id(conn))
    ...
    summary = sync_classroom(conn, client_archived, user_id=owner_id(conn))
...
def test_course_group_email_is_never_used_as_course_code(conn):
    ...
    # A synthetic alias. The test is about courseGroupEmail never becoming a
    # course code, so the value is irrelevant - and a real one copied out of
    # a Classroom account has no business in a public repository.
...
def test_sync_never_overwrites_personal_deadline(conn):
    ...
    repo.set_personal_deadline(conn, task_id=task["id"], personal_deadline="2026-09-05", user_id=owner_id(conn))
    ...
    assignment_v2["updateTime"] = "2026-08-25T00:00:00Z"
    client_v2 = FakeClient([COURSE], coursework={"course-1": [assignment_v2]})
    sync_classroom(conn, client_v2, user_id=owner_id(conn))
...
def test_completed_task_is_not_reopened_or_touched_if_source_item_disappears(conn):
    ...
    from ragra.db import repo
    repo.mark_completed(conn, task_id=task["id"], user_id=owner_id(conn))
